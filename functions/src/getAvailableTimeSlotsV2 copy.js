...
exports.getAvailableTimeSlotsV2 = functions.runWith({ memory: '1GB' }).https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Fetch existing bookings for the selected date for serviceable workers
        const bookingsQuery = admin.firestore().collection('bookings')
            .where('selectedDate', '==', dateString)
            .where('status', '==', 'confirmed')
            .where('workerId', 'in', serviceableWorkers.map(w => w.id)); // Optimization
        ...
        const bookingsByWorker = existingBookings.reduce((acc, booking) => {
            ...
            acc[booking.workerId].push({ start: busyStart, end: busyEnd });
            return acc;
        }, {});
            ...
            if (workerStartTime && workerEndTime && workerInterval) {
                ...
                if (dateString === today) {
                    ...
                    if (workerStartToday.isBefore(now)) {
                        ...
                        const workerEndToday = dayjs.tz(`${dateString} ${workerEndTime}`, UAE_TIMEZONE);
                ...
                slots.forEach(slot => {
                    ...
                    const workerBookings = bookingsByWorker[worker.id] || [];
                    const isBooked = workerBookings.some(booking => {
                        ...
                        return newSlotBusyStart.isBefore(booking.end) && newSlotBusyEnd.isAfter(booking.start);
                    });
