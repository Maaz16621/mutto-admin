...
export default function DiscountManager() {
  const [deleteModal, setDeleteModal] = useState({ open: false, discount: null });
  ...
  const fetchDiscounts = async () => {
    ...
    try {
      ...
      const discountList = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      setDiscounts(discountList);
    } catch (err) {
      toast({ title: "Error fetching discounts", status: "error", description: err.message });
    }
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    if (!form.name || !form.value || !form.startDate || !form.endDate) {
      toast({ title: "All fields except Service are required", status: "warning", position: "top-right" });
      return;
    }
    setLoading(true);
    try {
      ...
      if (selectedDiscount) {
        await updateDoc(doc(firestore, "discounts", selectedDiscount.id), discountData);
        toast({ title: "Discount updated", position: "top-right" });
      } else {
        await addDoc(collection(firestore, "discounts"), {
          ...discountData,
          createdAt: serverTimestamp(),
        });
        toast({ title: "Discount added", position: "top-right" });
      }
      fetchDiscounts();
      onClose();
      setForm({ name: "", type: "percentage", value: "", startDate: "", endDate: "", assignedServices: [] });
      setSelectedDiscount(null);
      setServiceInput("");
    } catch (err) {
      toast({ title: "Error saving discount", status: "error", description: err.message, position: "top-right" });
    }
    setLoading(false);
  };
  ...
  const openEdit = (discount) => {
    ...
    setForm(
      discount
        ? { ...discount, value: String(discount.value), assignedServices: discount.assignedServices || [] }
        : { name: "", type: "percentage", value: "", startDate: "", endDate: "", assignedServices: [] }
    );
    setServiceInput("");
    onOpen();
  };
  ...
  const handleDelete = async () => {
    const discount = deleteModal.discount;
    if (!discount) return;
    setLoading(true);
    try {
      await deleteDoc(doc(firestore, "discounts", discount.id));
      toast({ title: "Discount deleted", status: "success", position: "top-right" });
      fetchDiscounts();
    } catch (err) {
      toast({ title: "Error deleting discount", description: err.message, status: "error", position: "top-right" });
    }
        ...
        Cell: ({ row }) => (
          <Menu>
            ...
            <MenuList>
              ...
              <MenuItem onClick={() => setDeleteModal({ open: true, discount: row.original })}>Delete</MenuItem>
            </MenuList>
          </Menu>
        ),
      },
    ...
    <Flex direction="column" pt={{ base: "120px", md: "75px" }}>
      <Card overflowX={{ sm: "scroll", xl: "hidden" }} pb="0px">
        <CardHeader p="6px 0px 22px 0px">
          <Flex justify="space-between" align="center">
            <Heading size="md">Discount Management</Heading>
        ...
        <CardBody>
          <Flex mb={4} gap={4} flexWrap="wrap" align="center" justify="space-between">
            <InputGroup maxW="250px" boxShadow="sm">
              ...
              <Input placeholder="Search by name" value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} size="sm" bg="white" borderRadius="md" />
            </InputGroup>
          </Flex>
          {loading ? (
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered size="2xl">
        ...
        <ModalContent maxW="700px" minH="450px">
          <ModalHeader>{selectedDiscount ? "Edit Discount" : "Add Discount"}</ModalHeader>
          <ModalCloseButton />
            ...
            <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
              ...
              <FormControl mb={3}>
                <FormLabel display="flex" alignItems="center" gap={1}>
                  Assign to Services
                  <Tooltip label="Assign this discount to one or more services. Leave empty to apply to all services." placement="right" hasArrow>
      ...
      <Modal isOpen={deleteModal.open} onClose={() => setDeleteModal({ open: false, discount: null })} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Delete Discount</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            Are you sure you want to delete the discount <b>{deleteModal.discount?.name}</b>?
          </ModalBody>
          <ModalFooter>
            <Button colorScheme="red" mr={3} onClick={handleDelete} isLoading={loading}>
              Delete
            </Button>
            <Button variant="ghost" onClick={() => setDeleteModal({ open: false, discount: null })} isDisabled={loading}>Cancel</Button>
          </ModalFooter>
        </ModalContent>
      </Modal>
    </Flex>
