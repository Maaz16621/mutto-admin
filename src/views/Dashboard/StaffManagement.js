...
import { Box, Button, Flex, Heading, Input, Table, Thead, Tbody, Tr, Th, Td, InputGroup, InputLeftElement, IconButton, useToast, Spinner, Modal, ModalOverlay, ModalContent, ModalHeader, ModalBody, ModalFooter, ModalCloseButton, useDisclosure, FormControl, FormLabel, Select, Tooltip, Icon } from "@chakra-ui/react";
...
export default function StaffManagement() {
  ...
  const [form, setForm] = useState({ userName: "", email: "", password: "", role: "staff", status: "active" });
  ...
  const fetchStaff = async () => {
    setLoading(true);
    try {
      const q = query(collection(firestore, "staff"), where("role", "!=", "admin"));
  ...
  const openEdit = (staff) => {
    ...
    setForm(staff
      ...
      : { userName: "", email: "", password: "", role: "staff", status: "active" }
    );
    onOpen();
  };
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <Flex>
          ...
          <Button size="sm" onClick={() => handleToggleStatus(row.original)} colorScheme={row.original.status === "active" ? "red" : "green"} mr={2}>
            {row.original.status === "active" ? "Suspend" : "Activate"}
          </Button>
          <Button size="sm" colorScheme="red" onClick={() => setDeleteModal({ open: true, staff: row.original })}>Delete</Button>
        </Flex>
      ),
      disableSortBy: true,
      Filter: () => null,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5001/api/deleteUser', {
        method: 'POST',
  ...
  const filteredData = useMemo(() => {
    let data = staff;
    if (roleFilter) data = data.filter(row => row.role === roleFilter);
    ...
    return data;
  }, [staff, roleFilter, statusFilter, globalFilter]);
          ...
          <Flex mb={4} gap={4} flexWrap="wrap" align="center" justify="space-between">
            ...
            <Flex gap={2} align="center">
              <FormControl minW="150px" maxW="200px">
                <FormLabel fontSize="xs" mb={1} color="gray.600">Role</FormLabel>
                <Select placeholder="All Roles" value={roleFilter} onChange={e => setRoleFilter(e.target.value)} size="sm" bg="white" borderRadius="md" boxShadow="sm">
          ...
          <ModalContent maxH="90vh">
            ...
            <ModalBody overflowY="auto">
              ...
              <FormControl mb={3}>
                <FormLabel display="flex" alignItems="center" gap={1}>
                  Role
                  <Tooltip label="The staff member's role (staff or manager)." placement="right" hasArrow>
                    <span><Icon viewBox="0 0 20 20" color="gray.400" boxSize={4}><path fill="currentColor" d="M10 18a8 8 0 1 1 0-16 8 8 0 0 1 0 16zm0-14.5A6.5 6.5 0 1 0 10 17.5 6.5 6.5 0 0 0 10 3.5zm.75 10.25h-1.5v-1.5h1.5v1.5zm0-2.75h-1.5V7h1.5v4z"/></Icon></span>
                  </Tooltip>
                </FormLabel>
