...
export default function ProductCategoryManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (iconFile) {
        try {
          const iconRef = ref(storage, `product-category-icons/${Date.now()}_${iconFile.name}`);
      ...
      toast({ title: "Error saving product category", status: "error", description: err.message, position: "top-right" });
    }
    setLoading(false);
  };
    ...
    <Flex direction="column" pt={{ base: "120px", md: "75px" }}>
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <ModalContent maxH="90vh">
          <ModalHeader>{selectedCategory ? "Edit Product Category" : "Add Product Category"}</ModalHeader>
          <ModalCloseButton />
