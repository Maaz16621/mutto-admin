...
export default function ProductSubCategoryManager() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (iconFile) {
        try {
          const iconRef = ref(storage, `product-sub-category-icons/${Date.now()}_${iconFile.name}`);
      ...
      toast({ title: "Error saving product sub-category", status: "error", description: err.message, position: "top-right" });
    }
    setLoading(false);
  };
    ...
    <Flex direction="column" pt={{ base: "120px", md: "75px" }}>
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        ...
        <ModalContent maxH="90vh">
          <ModalHeader>{selectedSubCategory ? "Edit Product Sub-Category" : "Add Product Sub-Category"}</ModalHeader>
          <ModalCloseButton />
